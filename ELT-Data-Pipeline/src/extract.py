...
so the API doesn't crash, and saves the downloaded data to PostgreSQL.
It also uses PostgreSQL to remember which blocks were finished, so it can resume if interrupted.
"""
...
def get_watermark(pipeline_name: str) -> int:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT last_block FROM watermark WHERE pipeline_name = %s", (pipeline_name,)
    )
    row = cursor.fetchone()
    cursor.close()
    conn.close()
...
def set_watermark(pipeline_name: str, block: int):
    ...
    cursor.execute(
        """
        INSERT INTO watermark (pipeline_name, last_block) 
        VALUES (%s, %s)
        ON CONFLICT (pipeline_name) 
        DO UPDATE SET last_block = EXCLUDED.last_block, updated_at = CURRENT_TIMESTAMP
        """,
        (pipeline_name, block),
    )
    conn.commit()
...
def save_to_postgres(data_list: List[Dict[str, Any]], data_type: str):
    ...
    if data_type == "blocks":
        ...
        """
        values = [(d['hash'], d['block_number'], d['transaction_index'], d['from_address'], d['to_address'], d['value'], d['gas'], d['gas_price']) for d in data_list]

    elif data_type == "traces":
        query = """
            INSERT INTO traces (transaction_hash, block_number, trace_address, call_type, error, from_address, to_address, value)
            VALUES %s ON CONFLICT (transaction_hash, trace_address) DO NOTHING
        """
        values = [(d['transaction_hash'], d['block_number'], d['trace_address'], d['call_type'], d['error'], d['from_address'], d['to_address'], d['value']) for d in data_list]

    elif data_type == "logs":
        query = """
            INSERT INTO logs (transaction_hash, block_number, log_index, address, data, topics)
            VALUES %s ON CONFLICT (transaction_hash, log_index) DO NOTHING
        """
        values = [(d['transaction_hash'], d['block_number'], d['log_index'], d['address'], d['data'], d['topics']) for d in data_list]
